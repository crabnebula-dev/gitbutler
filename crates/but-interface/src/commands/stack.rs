...
pub fn update_branch_name(
    ...
    let project = gitbutler_project::get(params.project_id)?;
    let ctx = CommandContext::open(&project, ipc_ctx.app_settings.get()?.clone())?;
    gitbutler_branch_actions::stack::update_branch_name(&ctx, params.stack_id, params.branch_name, params.new_name)?;
    Ok(())
}
...
pub fn push_stack(
    ipc_ctx: &IpcContext,
    params: PushStackParams,
) -> Result<PushResult, Error> {
    let project = gitbutler_project::get(params.project_id)?;
    let ctx = CommandContext::open(&project, ipc_ctx.app_settings.get()?.clone())?;
    gitbutler_branch_actions::stack::push_stack(&ctx, params.stack_id, params.with_force, params.branch)
        .map_err(|e| e.into())
}
...
pub fn push_stack_to_review(
    ...
    let ctx = CommandContext::open(&project, ipc_ctx.app_settings.get()?.clone())?;
    let review_id =
        gitbutler_sync::stack_upload::push_stack_to_review(&ctx, &params.user, params.stack_id, params.top_branch)?;
